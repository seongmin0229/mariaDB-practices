...
-- 예) 2014년 07월 10일
select date_format(max(hire_date), '%Y년 %m월 %d일') as baby
from employees;
...
select date_format(curdate() - max(e.birth_date), '%y살') as '최고 어린 사원', date_format(curdate() - min(e.birth_date), '%y살') as '최고 연장자'
